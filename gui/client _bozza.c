...
//Variabile per il numero di motori da controllare
GtkEntryBuffer* buf_num_motors; 
...
void input(GtkWidget *widget, gpointer data){                     
	//gestore del bottone Set, invia l'input
...
void show_settings(GtkWidget *widget, gpointer data){
		...
		GtkWidget * set_window = gtk_window_new(GTK_WINDOW_TOPLEVEL);
		gtk_window_set_position(GTK_WINDOW(set_window), GTK_WIN_POS_CENTER);
		gtk_window_set_default_size(GTK_WINDOW(set_window), 200,50);//600, 80);
		gtk_window_set_title(GTK_WINDOW(set_window), "Settings");
		gtk_container_set_border_width(GTK_CONTAINER(set_window), 25);
		
		//Settiamo i vari widget
		GtkWidget *m_box = gtk_box_new(GTK_ORIENTATION_VERTICAL, 15);
		GtkWidget *set_box = gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 15);
		
		GtkWidget * num_label = gtk_label_new ("Motors:");
		buf_num_motors = gtk_entry_buffer_new("1",strlen("1"));
		GtkWidget * num_entry = gtk_entry_new_with_buffer(buf_num_motors);
		
		gtk_container_add (GTK_CONTAINER(set_box), num_label);
		gtk_container_add (GTK_CONTAINER(set_box), num_entry);
		gtk_container_add (GTK_CONTAINER(m_box), set_box);
		
		GtkWidget * apply_but = gtk_button_new_with_label("Apply");
		
		gtk_container_add (GTK_CONTAINER(m_box), apply_but);
		
		gtk_container_add (GTK_CONTAINER(set_window), m_box);
		
		gtk_window_set_resizable(GTK_WINDOW(set_window),FALSE);
		gtk_widget_show_all(set_window);
...
int main(int argc, char *argv[]) {
	...
	GtkWidget * settings_op = gtk_menu_item_new_with_label ("Settings");
	GtkWidget * help_op = gtk_menu_item_new_with_label ("Help");
	GtkWidget * about_op = gtk_menu_item_new_with_label ("About");
	gtk_menu_shell_append (GTK_MENU_SHELL (help_menu), settings_op);
	gtk_menu_shell_append (GTK_MENU_SHELL (help_menu), help_op);
	gtk_menu_shell_append (GTK_MENU_SHELL (help_menu), about_op);
	
	
	//Attiviamo l'azione "settings", "about" ed "help"
	g_signal_connect (settings_op, "activate", G_CALLBACK(show_settings), NULL);
